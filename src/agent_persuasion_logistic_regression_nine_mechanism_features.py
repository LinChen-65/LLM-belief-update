...
DEFAULT_OUT_DIR = "/data7/chenyitong/Winning_Arguments/final_v3_Experiment_Analysis_Results/agent_nine_mechanism_feature_logistic_regression"

DEFAULT_AGENT_RESULT_PATHS = [
    "/data7/chenyitong/Winning_Arguments/final_v3_results_deepseek-ai_DeepSeek-V3.json",
    "/data7/chenyitong/Winning_Arguments/final_v3_results_google_gemini-2.5-flash-lite.json",
    "/data7/chenyitong/Winning_Arguments/final_v3_results_gpt-4o-mini.json",
    "/data7/chenyitong/Winning_Arguments/final_v3_results_Pro_MiniMaxAI_MiniMax-M2.5.json",
    "/data7/chenyitong/Winning_Arguments/final_v3_results_Pro_zai-org_GLM-4.7.json",
    "/data7/chenyitong/Winning_Arguments/final_v3_results_Qwen_Qwen2.5-32B-Instruct.json",
    "/data7/chenyitong/Winning_Arguments/final_v3_results_Qwen_Qwen2.5-72B-Instruct.json",
]
...
FEATURE_SPECS = [
    ("ch_length", "Ch Length", "nine_mechanism", "up", 0.0001),
    ("ch_has_link", "Ch Has Link", "nine_mechanism", "up", 0.0001),
    ("ch_dissimilarity", "Ch Dissimilarity", "nine_mechanism", "up", 0.0001),
    ("ch_has_formatting", "Ch Has Formatting", "nine_mechanism", "up", 0.0001),
    ("ch_we_freq", "Ch We Freq", "nine_mechanism", "up", 0.0001),
    ("ch_def_freq", "Ch Def Freq", "nine_mechanism", "up", 0.0001),
    ("op_i_freq", "OP I Freq", "nine_mechanism", "up", 0.0001),
    ("op_def_freq", "OP Def Freq", "nine_mechanism", "up", 0.0001),
    ("op_length", "OP Length", "nine_mechanism", "up", 0.0001),
]
...
def nine_mechanism_features(reply_text, op_text, resources=None):
    """Measure the nine persuasion-mechanism features used in the classic comparison figure.

    Unit of analysis is one challenger reply paired with its original post.
    The implementation intentionally reuses the same low-level text cleaning,
    tokenization, URL detection, pronoun/article dictionaries, and Markdown
    formatting detectors used by the original 40-feature script.

    Feature definitions:
    - Ch Length: number of word tokens in the challenger reply.
    - Ch Has Link: binary indicator that the challenger reply contains at least one URL.
    - Ch Dissimilarity: 1 - Jaccard similarity between reply and OP word-type sets.
    - Ch Has Formatting: binary indicator for Markdown/list formatting in the reply.
    - Ch We Freq: first-person plural pronouns in reply divided by reply length.
    - Ch Def Freq: definite articles in reply divided by reply length.
    - OP I Freq: first-person singular pronouns in OP divided by OP length.
    - OP Def Freq: definite articles in OP divided by OP length.
    - OP Length: number of word tokens in the original post.
    """
    ...
    feats["ch_dissimilarity"] = 1.0 - sim if np.isfinite(sim) else np.nan
    feats["ch_has_formatting"] = int(
        has_bullet_list(reply_raw) == 1
        or has_numbered_list(reply_raw) == 1
        or count_markdown_bolds(reply_raw) > 0
        or count_markdown_italics(reply_raw) > 0
    )
...
def all_features(reply_text, op_text, resources):
    return nine_mechanism_features(reply_text, op_text, resources)
...
def infer_model_name(path):
    stem = Path(path).stem
    name = stem
    for prefix in ["final_new_results_", "final_results_", "results_"]:
...
def run_regressions(agent_df, out_dir, cov_type="cluster"):
    ...
    try:
        ...
        # ===== 新增的代码开始：输出 summary txt 文件 =====
        with open(Path(out_dir) / "agent_logistic_regression_model_summary.txt", "w", encoding="utf-8") as f:
            f.write(str(pool_res.summary()))
            f.write("\n\nNotes:\n")
            f.write("- Dependent variable: agent_delta_awarded, where success reply = 1 and failure reply = 0.\n")
            f.write("- Features are the nine mechanism features: Ch Length, Ch Has Link, Ch Dissimilarity, Ch Has Formatting, Ch We Freq, Ch Def Freq, OP I Freq, OP Def Freq, and OP Length.\n")
            ...
            f.write("- This summary represents the POOLED model WITH model fixed effects to account for multiple agent models.\n")
        # ===== 新增的代码结束 =====
...
def main():
    ...
    metadata = {
        ...
        "regression_note": "Dependent variable is agent_delta_awarded. Regressions use the nine mechanism features only. Pooled regression includes model fixed effects; per-model regressions are robustness checks.",
    }
    ...
    with open(out_dir / "quick_report.txt", "w", encoding="utf-8") as f:
        ...
        f.write("Feature set: nine_mechanism_features; n_features=9\n")
        ...
        f.write(f"Model summary text file: {out_dir / 'agent_logistic_regression_model_summary.txt'}\n")
    ...
    print(f"Model summary text file: {out_dir / 'agent_logistic_regression_model_summary.txt'}")
